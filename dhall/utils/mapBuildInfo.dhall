...
let mapOptional = https://prelude.dhall-lang.org/v10.0.0/Optional/map

let mapList = https://prelude.dhall-lang.org/v10.0.0/List/map
		...
		, executables =
			mapList
			  ...
			  (   λ(namedGuardedExecutable : NamedGuardedExecutable)
				→ { name = namedGuardedExecutable.name
				  , executable =
					  mapGuarded
						types.Executable
		...
		, test-suites =
			mapList
			  ...
			  (   λ(namedGuardedTestSuite : NamedGuardedTestSuite)
				→ { name = namedGuardedTestSuite.name
				  , test-suite =
					  mapGuarded
						types.TestSuite
		...
		, benchmarks =
			mapList
			  ...
			  (   λ(namedGuardedBenchmark : NamedGuardedBenchmark)
				→ { name = namedGuardedBenchmark.name
				  , benchmark =
					  mapGuarded
						types.Benchmark
		...
		, foreign-libraries =
			mapList
			  ...
			  (   λ(namedGuardedForeignLibrary : NamedGuardedForeignLibrary)
				→ { name = namedGuardedForeignLibrary.name
				  , foreign-lib =
					  mapGuarded
